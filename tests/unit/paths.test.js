...
import { getOrgRoot, getDataPath, getStylesDir, getPresetsDir, getDefinitionsDir, getDefinitionsPath, getProjectRoot } from '../../.claude/orgframework/lib/paths.js';

describe('paths', () => {
  it('getOrgRoot() returns absolute path ending with orgframework', () => {
    const root = getOrgRoot();
    expect(root).toContain('orgframework');
    expect(root).toMatch(/^\//); // absolute path
  });

  it('getDataPath() joins subpath onto org root', () => {
    const dataPath = getDataPath('styles');
    expect(dataPath).toContain('orgframework');
  ...
  it('getProjectRoot() returns path two levels above lib/', () => {
    ...
    expect(projectRoot).toMatch(/orgframework$/);
  });
});
